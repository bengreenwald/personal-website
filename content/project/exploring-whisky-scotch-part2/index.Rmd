...
# Next Steps
We've compared variables such as `age`, `ABV`, `price`, and `points` and learned about whisky `type` from scotch sourced all across Scotland. Next, Part 3 will focus on the regionality of scotch to better understand the distribution and clustering of distilleries across different regions of Scotland -- and what makes each region's whisky so special and unique.